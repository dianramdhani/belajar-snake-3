...
// src/ai/genetic/test-user-weights2.ts
...
var visionFrontWall = getVision([{ x: 10, y: 19 }], { x: 5, y: 5 }, "DOWN", 20);
var outFrontWall = nn.predict(visionFrontWall);
var maxIdxFront = outFrontWall.indexOf(Math.max(...outFrontWall));
console.log("Wall in front. Output:", outFrontWall, "Decision:", ["FORWARD", "RIGHT", "LEFT"][maxIdxFront]);
var visionFoodRight = getVision([{ x: 10, y: 10 }], { x: 15, y: 10 }, "UP", 20);
var outFoodRight = nn.predict(visionFoodRight);
var maxIdxFoodRight = outFoodRight.indexOf(Math.max(...outFoodRight));
